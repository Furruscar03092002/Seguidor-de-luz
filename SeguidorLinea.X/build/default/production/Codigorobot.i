...
INISYS:
    ...
    BSF STATUS, 5 ; Bank1
    ; Modificar TRIS
    BSF TRISC, 0 ; SC2 <- entrada
    BSF TRISC, 1 ; SC1 <- entrada
    BSF TRISC, 2 ; SB2<- entrada
    BSF TRISC, 3 ; SB1 <- entrada
    BSF TRISC, 4 ; SA <- entrada
    BCF TRISD, 0 ; ML1 <- salida
    BCF TRISD, 1 ; ML2 <- salida
    BCF TRISD, 2 ; MR1 <- salida
    BCF TRISD, 3 ; MR2 <- salida
    BCF TRISD, 4 ; LL <- salida
    BCF TRISD, 5 ; LR <- salida
    BCF TRISD, 6 ; DYR <- salida
    ; Regresar a banco 0
